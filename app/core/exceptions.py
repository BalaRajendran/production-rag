...
class RateLimitError(RAGError):
    ...
    def __init__(
        ...
        super().__init__(
            message=message, status_code=status.HTTP_429_TOO_MANY_REQUESTS, details=error_details
        )
...
def rag_exception_to_http_exception(exc: RAGError) -> HTTPException:
    """
    ...
    """
    detail = {"error": exc.message, **exc.details}
    ...
    return HTTPException(
        status_code=exc.status_code, detail=detail, headers=headers if headers else None
    )
