...
class QueryGenerationService:
    ...
    async def generate_queries(
        ...
        if not conversation_history or len(conversation_history) == 0:
            return [
                ...
                GeneratedQuery(
                    query=self._extract_keywords(current_query),
                    query_type="keyword"
                )
            ]
    ...
    def _build_query_generation_prompt(
        ...
        prompt = f"""Given the conversation history and current query, generate 3-5 semantic search queries that capture different aspects of what the user is looking for.
        ...
        try:
            ...
            for line in lines:
                ...
                if line and len(line) > 10:  # Reasonable query length
                    queries.append(
                        GeneratedQuery(query=line, query_type="semantic")
                    )
